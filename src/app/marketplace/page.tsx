    ...
    <Card className="overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 flex flex-col">
        <CardHeader className="p-0">
             <Carousel className="w-full">
                <CarouselContent>
                    {vehicle.imageUrls && vehicle.imageUrls.length > 0 ? vehicle.imageUrls.map((url, index) => (
                         <CarouselItem key={index}>
                            <div className="relative h-56 w-full bg-muted flex items-center justify-center">
                                <Image src={url} alt={`${vehicle.brand} ${vehicle.model} - Imagen ${index + 1}`} fill className="object-cover" />
                            </div>
                        </CarouselItem>
                    )) : (
                         <CarouselItem>
