      ...
      <div className="container px-4 md:px-6">
        <div className="flex flex-col items-center justify-center space-y-4 text-center">
          <div className="space-y-2">
            <div className="inline-block rounded-lg bg-secondary px-3 py-1 text-sm">Workshop Locator</div>
            <h2 className="text-3xl font-bold tracking-tighter sm:text-5xl font-headline text-primary">Find a Trusted Mechanic Near You</h2>
            <p className="max-w-[900px] text-muted-foreground md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed">
              Browse our network of registered workshops. Filter by services to find the perfect match for your car's needs.
            </p>
          </div>
        </div>
        <div className="mx-auto max-w-6xl mt-12">
            <div className="flex flex-col sm:flex-row gap-4 mb-8 p-4 border rounded-lg bg-card sticky top-20 z-10 backdrop-blur-md shadow-lg">
                <div className="flex-1 relative">
                    ...
                    <Input id="search" placeholder="Search by name, e.g., Precision Auto" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
                </div>
                <div className="flex items-center justify-center sm:justify-end">
                    <div className="flex items-center space-x-2">
                        <Switch id="obd-filter" checked={showObdOnly} onCheckedChange={setShowObdOnly} />
                        <Label htmlFor="obd-filter" className="text-nowrap">OBD-II Scanners Only</Label>
                    </div>
                </div>
            </div>
